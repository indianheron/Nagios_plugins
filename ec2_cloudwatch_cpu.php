...
Usage 		: php ec2_cloudwatch_cpu.php -a <access_key> -s <secrete_key> -i <instance_public_url> -w <warning_threshold> -c <critical_threshold>
-------------------------------------------------------------------------------------------------------------*/
...
if(count($argv) < 4)
{
	echo "USAGE: $argv[0] -a <access_key> -s <secret_key> -i <ec2_Public_URL> [-w warning_level] [-c critical level]\n";
	exit(3);
}
else
{
...
class cloudWatch_plugin
{
	...
	public function Get_Stats_CPUUtilization()
	{
		...
		$this->ass_array['CPUUtilization']['Average'] 	  = $tmp[5][0];
                settype($this->ass_array['CPUUtilization']['Average'], "float");
	}
	...
	public function Print_vals()
	{
		...
		$status = 0;
                $lable = "STATS OK :";
		if($this->ass_array['CPUUtilization']['Average'] > $this->options['w'])
		{	
			$status = 1;
			$lable = "STATS Warning :";	
		}
		if($this->ass_array['CPUUtilization']['Average'] > $this->options['c'])
                {
                        $status = 2;
                        $lable = "STATS Critical :";
                }
		$output = "$lable CPUUtilization - ".$this->ass_array['CPUUtilization']['Average']."% | CPUUtilization=".$this->ass_array['CPUUtilization']['Average']."%\n";
